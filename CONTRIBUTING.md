...
- Feel free to commit as many as needed. But changes SHOULD NOT be done on the `main` branch.
- Always develop on the `dev` branch or a new branch created from `dev`.
- Only open a pull request to `main` once you have considerable number of changes (we can't waste our CI/CD credits).
- President approval required for merging to `main`.
